...
func TestNode_Fail(t *testing.T) {
	...
	t.Run("Test change download target on fail", func(t *testing.T) {
		...
		node1.addRequest(newRouteRequest(0, "key", node1, []INode{node3}, []INode{}, node2))

		node2.AddToStore("key", "value")
		node3.AddToStore("key", "value")
	...
	t.Run("Test haver fail", func(t *testing.T) {
		node1 := NewNode(1, 1, nil, func(with INode) (int, int) { return 1, 3 })
		node2 := NewNode(1, 1, nil, func(with INode) (int, int) { return 1, 3 })
		// node3 := NewNode(1, 1, nil, func(with INode) (int, int) { return 1, 3 })
		...
		node1.getNetworkFriends = func() []INode { return []INode{node2} }
		node2.getNetworkFriends = func() []INode { return []INode{node1} }
		node2.getFailChance = func(method Method) float64 {
			if method == ReceiveDownloadMethod {
				return 1
			} else {
				return 0
			}
		...
		_, ok := node1.ReceivedKey("key")

		if ok {
			t.Fatal("Failed did returned info")
		}
	})

	t.Run("Test haver fail complex", func(t *testing.T) {
		node1 := NewNode(1, 1, nil, func(with INode) (int, int) { return 1, 1 })
		node2 := NewNode(1, 1, nil, func(with INode) (int, int) { return 1, 1 })
		node3 := NewNode(1, 1, nil, func(with INode) (int, int) { return 1, 1 })
		node4 := NewNode(1, 1, nil, func(with INode) (int, int) { return 1, 1 })
		node5 := NewNode(1, 1, nil, func(with INode) (int, int) { return 1, 1 })

		node1.getNetworkFriends = func() []INode { return []INode{node2, node3, node4} }
		node2.getNetworkFriends = func() []INode { return []INode{node1, node3, node4} }
		node3.getNetworkFriends = func() []INode { return []INode{node1, node2, node4, node5} }
		node4.getNetworkFriends = func() []INode { return []INode{node1, node2, node3} }
		node5.getNetworkFriends = func() []INode { return []INode{node3} }

		node1.getFailChance = func(method Method) float64 {
			if method == ReceiveDownloadMethod {
				return 1
			} else {
				return 0
		...
		for i, n := range []INode{node1, node2, node3, node4, node5} {
			t.Logf("Node%d has a pointer of %p\n", i+1, n)
		}

		node1.AddToStore("key", "val")

		node4.StartSearch("key")
		node5.StartSearch("key")

		// t.Logf("Here we have request : %+v\n", node3.routeRequests)

		time.Sleep(300 * time.Millisecond)

		_, ok1 := node4.ReceivedKey("key")
		_, ok2 := node5.ReceivedKey("key")
...
func TestNode(t *testing.T) {
	...
	t.Run("Test end-to-end two messages", func(t *testing.T) {
		...
		ok, error := failAtButSuccedAt(func() bool {
			v1, ok1 := node1.ReceivedKey("key")
			v2, ok2 := node2.ReceivedKey("key")
